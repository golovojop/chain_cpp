...
int main() {
    ...
    */
    ...
    tx.locktime = 0;
        ...
        bc_print_bytes("tx.outputs[1].script: ", tx.outputs[1].script, tx.outputs[0].script_len);
    ...
    bn_priv = BN_new();
    BN_bin2bn(priv_bytes, 32, bn_priv);
    ...
    // 2.3 Конвертируем сигнатуру в формат DER. Дополнительный байт для флага SIGHASH_ALL 
    der_len = ECDSA_size(key) + 1;
...
//    der[der_len - 1] = SIGHASH_ALL;
    ...
    bc_txin_create_p2pkh(&ins[0], &outpoint, der_sz, pub_my, BC_SIGHASH_ALL);
