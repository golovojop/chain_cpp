...
int main() {
    ...
    // Короче, ноеобходимо инициализировать нулями вообще все, иначе там остается мусор,
    // который затем не позволяет нормально работать с переменными UINT64_T
    ...
    */
    const char txid_src[]	= "901447fb1a8b237aa3aedd5a2b9c4980fb7eb11197e928e221358cb7e53f99d8";


    /* --------------------------------------------------------------------------------------------------------------------*/
    /* Сначала создадим ТЕЛО ДЛЯ СИГНАТУРЫ. Это фейковая транзакция, тело которой будет материалом для цифровой подписи    */
    /* --------------------------------------------------------------------------------------------------------------------*/

    /*
	!!!!!!!!!!!!!!!!!!
	
	1 BTC равен 100 000 000 Satoshi. Поэтому 146700000 сатоши - это 146700000 * (1/100 000 000) = 1,467 BTC
    ...
    */
    ...
    /* output 0 (1.3 BTC) */
    bc_txout_create_p2pkh(&outs, 146700000, addr_to);
    /* output of source tx */
    bc_txout_create_p2pkh(&prev_outs, 146770000, addr_me);
    /* input from utxo (1,46700000 BTC) */
    bc_outpoint_fill(&outpoint, txid_src, 1);
    /* Это модифицированный input, ссылающийся на источник средств нашей транзакции*/
...
*/
    ...
    // 1.2 Поместим в него наш приватный ключ priv_bytes[].
    // Сначала переводим массив в BIGNUM, затем BIGNUM в EC_KEY
    bn_priv = BN_new();
    // Так как наш приватный ключ представлен в виде строки шестнадцатеричных цифр,
    // то необходимо эту строку сконвертить в реальные байты.
    ...
    // 2.1 Получаем хэш сообщения. Нужен hash256 (Не sha256 !!!)
    //printf("msg len: %d\n", msg_len);
    //bc_print_bytes("msg: ", msg, msg_len);
    
    /*
	БЛЯ !!! Вот из-за этого я долго не мог понять почему не работает !!!
	Нужен hash256, а не sha256
    */
...
//    printf("s: %s\n", BN_bn2hex(signature->s));
    ...
    // Флаг  SIGHASH_ALL будет добавлен к сигнатуре в вызове bc_txin_create_p2pkh()
